...
#
# Taken from _TURN_DETECTION, not from the session template: the phone agent
# accepts calls with turn_detection set to None so that nothing can cut its
# opening, and switches this on over its control WebSocket a few seconds later.
# The demo has no control socket — the browser talks straight to OpenAI — so
# copying the template would leave it permanently deaf, with nothing able to
# turn detection on. It gets the tuned VAD from the start instead; its greeting
# is not a phone line and has no noise to be cut by.
_DEMO_TURN_DETECTION = dict(_TURN_DETECTION)
...
_DEMO_NOTE_IT = (
    ...
    "(es. 'Buongiorno, mi chiamo Apollonia, sono un'assistente virtuale. Come\n"
    "posso aiutarla?'). Dichiarare di essere un assistente virtuale è\n"
    "obbligatorio anche qui e non va mai omesso. Qui però NON dire di quale\n"
    "studio o azienda sei l'assistente: in questa demo non rappresenti nessuna\n"
    "agenzia, e ApollonIA è il tuo stesso nome: lo diresti due volte. NON\n"
    "aprire elencando cosa non puoi fare né dicendo che è una demo: spiega i\n"
    "limiti della demo SOLO se il visitatore chiede qualcosa che in questa\n"
    "demo non puoi fare."
)
...
_DEMO_NOTE_SK = (
    ...
    "virtuálna asistentka. Ako vám môžem pomôcť?'). Vyhlásenie, že ste\n"
    "virtuálna asistentka, je povinné aj tu a nikdy sa nevynecháva. TU však\n"
    "NEHOVORTE, čia asistentka ste: v tejto ukážke nezastupujete žiadnu\n"
    "kanceláriu a ApollonIA je vaše vlastné meno, povedali by ste ho dvakrát.\n"
